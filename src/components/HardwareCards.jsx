    ...
    <div className="grid gap-4 sm:grid-cols-2">
      {hardwareItems.map((item) => {
        ...
        return (
          <article
            key={item.name}
            className="group relative overflow-hidden rounded-2xl border border-slate-700 bg-slate-950/70 p-5"
          >
            ...
            {item.hover ? (
              <div className="pointer-events-none absolute inset-x-0 bottom-0 translate-y-full bg-cyan-500/15 p-3 text-xs text-cyan-100 transition duration-300 group-hover:translate-y-0">
                {item.hover}
              </div>
            ) : null}
          </article>
